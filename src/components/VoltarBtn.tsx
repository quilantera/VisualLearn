...
export function VoltarBtn(){
    ...
    return(
        ...
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content tabIndex={0} className="bg-slate-50 rounded-xl z-50 shadow-lg fixed top-1/2 left-1/2 w-[42rem] max-w-4/6  px-7 py-[30px] dark:bg-black dark:border-white dark:border-2 " style={{"transform": "translate(-50%, -50%)","animation":"contentShow 150ms cubic-bezier(0.16, 1, 0.3, 1)" }}>
            ...
            <div style={{ display: 'flex', gap: 25, justifyContent: 'flex-end' }}>
              <AlertDialog.Cancel asChild>
                <button className="bg-zinc-300  duration-300 rounded font-medium px-[20px] py-[12px] text-gray-950 shadow-lg hover:bg-zinc-400 dark:bg-black dark:text-white dark:border-4 dark:border-sky-900 dark:hover:scale-105" aria-label='botão cancelar'>cancelar</button>
              </AlertDialog.Cancel>
              <AlertDialog.Action asChild>
                <Link href="/" className="bg-green-700 text-white px-[20px] py-[12px] rounded shadow-lg hover:bg-green-900 duration-300 dark:bg-black dark:border-4 dark:border-red-900 dark:hover:scale-105"  aria-label='botão sair da atividade' >sim, desejo sair</Link>
              </AlertDialog.Action>
            </div>
          </AlertDialog.Content>
        </AlertDialog.Portal>
